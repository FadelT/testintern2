...
def optimise_total(theta,data,t,modele_total,Delta_max):
    #function cout=optimise_total(theta,data,t,modele_total,Delta_max)

    theta[0] = max(data.iloc[0],theta[0])#; % Temp0 >= 1er point de la courbe
    theta[1] = max(2,theta[1])#; % Amp1 >= 2
    theta[1] = min(Delta_max,theta[1])#; %Amp1 <= Tmax-Temp0
    theta[2] = max(0,theta[2])#; % t0 >= 1
    theta[3]=max(0.1,theta[3])#; % alpha >= 0.1
    theta[3]=min(5,theta[3])#; % alpha <= 5
    theta[4]=max(0,theta[4])#; % Amp2 >= 0
    theta[5] = max(theta[2]+1,theta[5])#; % t1 >= t0+1
    theta[6]=max(0,theta[6])#; % beta >= 0

    #% Récupérer les variables en paramètres
    T0=theta[0]
    Amp1=theta[1]
    t0=theta[2]
    alpha=theta[3]
    ...
    #% Fonction totale à otimiser
    modele=modele_total(T0,Amp1,alpha,t,t0,Amp2,beta,t1)
    #print(modele)
    #% Calculer l'erreur entre le modèle et le tirage de base
    erreur=modele-data
    #% Calculer le coût à optimiser
    cout=np.mean(erreur**2)


    return cout

def modele_total(T0,Amp1,alpha,t,t0,Amp2,beta,t1):
    d=0
    firs=np.multiply((1-np.exp(-alpha*(t-t0))),(1+np.tanh(11.57*(t1-t)))/2) + np.multiply((1-np.exp(-alpha*(t1-t0))),(1+np.tanh(11.57*(t-t1)))/2)
    total=np.multiply(Amp1,np.multiply(firs,(1+np.tanh(11.57*(t-t0)))/2))
    tot=total-np.multiply(np.multiply(Amp2,(1-np.exp(-beta*(t-t1)))),(1+np.tanh(11.57*(t-t1)))/2)+T0
    return tot


def analyse_tir(data):
...
df.to_csv('/users/promo2021/nkassinga/testintern2/don_tirV3.csv"')
print(type(df))